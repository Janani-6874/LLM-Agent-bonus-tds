...
class LLM TDS {
    ...
    async init() {
        try {
            ...
            console.log(`LLM TDS v${this.version} initialized successfully`);
        } catch (error) {
            console.error('Failed to initialize LLM TDS:', error);
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'LLM TDS', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to LLM TDS! How can I assist you?');
        }
    }
